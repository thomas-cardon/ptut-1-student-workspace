...
export function usePosts(user, type = 0, module) {
  return useSWR(`/api/posts?type=${type}${module ? '&module=' + module : ''}` + (user?.userType === 0 && user?.group?.id ? '&filterByGroup=' + user?.group?.id : ''), fetcher);
}
