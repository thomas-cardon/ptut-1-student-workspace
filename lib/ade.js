 ...
 * @return {[Object]} [Les blocs de cours]
 */
export function parseCalendar(user, year) {
  if (!user || !year || typeof window === `undefined`) return [];

  return sessionStorage.getItem(`${user.school}/${user.degree}/${year || user.year}`) ? JSON.parse(sessionStorage.getItem(`${user.school}/${user.degree}/${year || user.year}`)).map(x => ({
    ...x,
...
export async function getCurrentCourse(user, year) {
  const calendar = parseCalendar(user, year);

  if (calendar.length === 0) return console.error("Le calendrier est vide");
  else console.dir(calendar);

  const course = calendar.find(x => isWithinInterval(new Date(), {
    start: x.start,
    end: x.end
  }));
...
export async function useADE(user, school = 'IUT Aix-en-Provence', degree = 'Informatique', year = '1ère année Groupe 3A') {
  ...
  if (!sessionStorage.getItem(`${school}/${degree}/${year}`)) {
    ...
    try {
      ...
      sessionStorage.setItem(`${school}/${degree}/${year}`, JSON.stringify(bakeClasses(await r.text())));
