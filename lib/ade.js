 ...
 * @return {String}        [L'URL]
 */
export function getURL(user, resourceId) {
  console.dir(arguments);
  
  if (!user.schedule.resourceId && !resourceId) throw Error('[ADE] Resource ID manquant');
  return user.schedule.url + '&resources=' + (resourceId || user.schedule.resourceId);
}
 ...
 * @return {[Object]} [Les blocs de cours]
 */
export function parseCalendar({ user, calendar, resource }) {
  ...
  return JSON.parse(sessionStorage.getItem(`${user.school}/${user.degree}/${resource || user.group?.id || user.schedule.resourceId}`) || '[]').map(deserialize);
}

/**
 * [Récupère les données enregistrées dans le navigateur, et de-sérialise les strings en dates; Si l'objet n'existe pas, il tentera de le récupérer avec useADE]
 * @return {[Object]} [Les blocs de cours]
 */
export async function parseCalendarAsync({ user, calendar, resource }) {
  if (typeof window === `undefined`) return [];
  ...
  const array = sessionStorage.getItem(`${user.school}/${user.degree}/${resource || user.group?.id || user.schedule.resourceId}`);
  return array ? JSON.parse(array).map(deserialize) : await useADE(user, { resource });
}

export async function getNextCourse({ user, calendar }) {
  calendar = parseCalendar({ user, calendar });
...
export async function getCurrentCourse({ user, calendar }) {
  calendar = parseCalendar({ user, calendar });
...
export async function useADE(user, options = {}) {
  ...
  let calendar = sessionStorage.getItem(`${user.school}/${user.degree}/${options.resource || user.group?.id || user.schedule.resourceId}`);

  if (!calendar) {
    const url = `${process.env.NEXT_PUBLIC_URL_PREFIX}/api/schedule/fetch?resource=${options.resource || user.group?.id || user.schedule.resourceId}`;
    ...
    try {
      ...
      sessionStorage.setItem(`${user.school}/${user.degree}/${options.resource || user.group?.id || user.schedule.resourceId}`, JSON.stringify(calendar));
      window.dispatchEvent(new Event('storage'));

      return parseCalendar({ calendar });
    } catch (error) {
        console.error(error);
        sessionStorage.removeItem(`${user.school}/${user.degree}/${options.resource || user.group?.id || user.schedule.resourceId}`);
        console.log('[ADE] Les données récupérées sont incompatibles. Annulation.');
    }
  }
