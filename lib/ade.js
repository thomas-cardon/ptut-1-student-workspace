 ...
 * @return {[Object]} [Les blocs de cours]
 */
export function parseCalendar(user) {
  if (!user || typeof window === `undefined`) return [];

  return sessionStorage.getItem(user.userId + '/ade_data') ? JSON.parse(sessionStorage.getItem(user.userId + '/ade_data')).map(x => ({
    ...x,
...
export async function useADE(user, school = 'IUT Aix-en-Provence', degree = 'Informatique', year = '1ère année Groupe 3A') {
  ...
  if (!sessionStorage.getItem(`${user.userId}/ade_data`)) {
    ...
    try {
      ...
      sessionStorage.setItem(`${user.userId}/ade_data`, JSON.stringify(bakeClasses(await r.text())));
    } catch (error) {
        console.error(error);
        sessionStorage.removeItem(`${user.userId}/ade_data`);
