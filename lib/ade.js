...
export function parseCalendar({ calendar, user, degree }) {
  ...
  return JSON.parse(sessionStorage.getItem(`${user.school}/${user.degree}/${degree || user.degree}`) || '[]').map(deserialize);
}
...
export async function useADE(user, options = {}) {
  ...
  let calendar = sessionStorage.getItem(`${user.school}/${user.degree}/${options.degree || user.degree}`);

  if (!calendar) {
    const url = `${process.env.NEXT_PUBLIC_URL_PREFIX}/api/schedule/get-ade-data`;
    console.log(`[ADE] Fetching for ->`, url);
    ...
    try {
      ...
      sessionStorage.setItem(`${user.school}/${user.degree}/${options.degree || user.degree}`, JSON.stringify(calendar));
