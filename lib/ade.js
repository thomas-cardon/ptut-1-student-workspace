...
export function parseCalendar({ calendar, user, year }) {
  if ((!calendar || (!user && !year)) && typeof window === `undefined`) return [];

  console.dir(arguments);

  return JSON.parse(calendar || (user && year ? sessionStorage.getItem(`${user.school}/${user.degree}/${year || user.year}`) : '[]')).map(x => ({
    ...x,
...
export async function getCurrentCourse({ user, year, calendar }) {
  if (!calendar) calendar = parseCalendar({ user, year });

  const course = calendar.find(x => {
    try {
