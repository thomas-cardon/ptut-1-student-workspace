...
export function parseCalendar({ user, calendar, resource }) {
  ...
  return JSON.parse(sessionStorage.getItem(`${user.school}/${user.degree}/${resource || user.schedule.resourceId}`) || '[]').map(deserialize);
}
...
export async function parseCalendarAsync({ user, calendar, resource }) {
  ...
  const array = sessionStorage.getItem(`${user.school}/${user.degree}/${resource || user.schedule.resourceId}`);
...
export async function useADE(user, options = {}) {
  ...
  let calendar = sessionStorage.getItem(`${user.school}/${user.degree}/${options.resource || user.schedule.resourceId}`);

  if (!calendar) {
    const url = `${process.env.NEXT_PUBLIC_URL_PREFIX}/api/schedule/fetch?resource=${options.resource || user.schedule.resourceId}`;
    console.log(`[ADE] Fetching for ->`, options.resource || user.schedule.resourceId);
    ...
    try {
      ...
      sessionStorage.setItem(`${user.school}/${user.degree}/${options.resource || user.schedule.resourceId}`, JSON.stringify(calendar));
      window.dispatchEvent(new Event('storage'));

      return parseCalendar({ calendar });
    } catch (error) {
        console.error(error);
        sessionStorage.removeItem(`${user.school}/${user.degree}/${options.resource || user.schedule.resourceId}`);
        console.log('[ADE] Les données récupérées sont incompatibles. Annulation.');
    }
  }
