...
async function handler(req, res) {
  ...
  try {
    if (req.method === 'DELETE' && user.userType == 2) { // Suppression
      const data = await query(`DELETE FROM subjects WHERE module = ?`, req.query.moduleId);
      ...
      const results = await query(
        `
        INSERT INTO subjects (module, name, color)
        VALUES (?, ?, ?)
        `,
      ...
      const data = await query(`SELECT * FROM subjects WHERE module = ?`, req.query.moduleId);
