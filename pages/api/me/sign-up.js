...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.body.teacher === true && !req.body.resourceId) return res.status(400).json({ error: 'RESOURCEID_MISSING', message: "Le paramètre resourceId est manquant.", success: false });

    const hashed = await hash(req.body.password);

    await query(`
      INSERT INTO users (firstName, lastName, email, hash, birthDate, userType, degreeId, groupId, resourceId)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [req.body.firstName, req.body.lastName, req.body.email, hashed, req.body.birthDate, 0, req.body.degree, req.body.group, req.body.teacher === true ? req.body.resourceId : null]);
